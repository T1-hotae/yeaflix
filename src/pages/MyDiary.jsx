...
export default function MyDiary() {
  ...
  const filtered = filter === 'all'
    ? diaries
    : diaries.filter((d) => d.rating === Number(filter));
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-cinema-muted text-sm mt-1">
              총 {diaries.length}편의 영화를 기록했습니다.
            </p>
          </div>
            ...
            {['all', '5', '4', '3', '2', '1'].map((f) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition ${
                  filter === f
                    ? 'bg-cinema-gold text-black border-cinema-gold font-bold'
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-32 bg-cinema-card rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-cinema-muted">
              {filter === 'all'
                ? '아직 작성된 일기가 없습니다. 영화를 보고 감상을 남겨보세요!'
                : '해당 별점의 일기가 없습니다.'}
            </p>
            {filter === 'all' && (
              <Link
                to="/"
                className="inline-block mt-4 bg-cinema-gold text-black font-semibold px-6 py-2.5 rounded-full hover:bg-yellow-400 transition text-sm"
              >
                영화 둘러보기
              </Link>
            )}
            ...
            {filtered.map((diary) => (
              ...
              >
                ...
                <div className="w-16 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-cinema-surface">
                  {diary.moviePoster ? (
                    <img
                      src={getPosterUrl(diary.moviePoster, 'w92')}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="text-white font-semibold group-hover:text-cinema-gold transition line-clamp-1">
                      {diary.movieTitle}
                    </h3>
                  ...
                  {diary.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {diary.tags.slice(0, 5).map((tag) => (
                        <span
                          key={tag}
                          className="text-xs px-2 py-0.5 rounded-full bg-cinema-gold/10 text-cinema-gold/80"
                        >
                          #{tag}
                        </span>
                      ))}
                      {diary.tags.length > 5 && (
                        <span className="text-xs text-cinema-muted">+{diary.tags.length - 5}</span>
                      )}
                    </div>
                  )}
                </div>
