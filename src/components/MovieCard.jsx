    ...
    >
      ...
      <div className="aspect-[2/3] overflow-hidden bg-cinema-surface">
        {poster ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-cinema-muted text-4xl">
            🎬
          </div>
        )}
      </div>
      ...
      {/* 일기 작성 뱃지 */}
      {hasDiary && (
        <span className="absolute top-2 right-2 bg-cinema-gold text-black text-xs font-bold px-2 py-0.5 rounded-full shadow">
          ✍️
        </span>
      )}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end p-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-cinema-gold text-xs">★ {formatRating(movie.vote_average)}</span>
